...
const isMobileBrowser = () =>
  ...
  /Android|iPhone|iPad|iPod|SamsungBrowser/i.test(navigator.userAgent);
...
export function ImageUpload({ 
  ...
  const uploadFile = async (selectedFile: File) => {
    ...
    try {
      ...
      keepFilePickerGuardAfterSuccessfulUpload();
        ...
        if (isMobileBrowser()) {
          window.setTimeout(() => onChange(result.path), 250);
          window.setTimeout(() => onChange(result.path), 1_000);
        }
  ...
  const openFilePicker = async (event?: React.MouseEvent<HTMLElement>) => {
    ...
    extendFilePickerGuard(5 * 60 * 1000);
